...
const minTestReadBufferSize = 16

var bufsizes = []int{
	0, minTestReadBufferSize, 23, 32, 46, 64, 93, 128, 1024, 4096,
}
...
func TestUnreadByteOthers(t *testing.T) {
	...
	readers := []func(*Reader, byte) ([]byte, error){
		(*Reader).ReadBytes,
		(*Reader).ReadSlice,
	...
	for rno, read := range readers {
		...
		r := NewReaderSize(&buf, minTestReadBufferSize)
...
func TestBufferFull(t *testing.T) {
	...
	buf := NewReaderSize(strings.NewReader(longString), minTestReadBufferSize)
...
func TestPeek(t *testing.T) {
	p := make([]byte, 10)
	// string is 16 (minTestReadBufferSize) long.
	buf := NewReaderSize(strings.NewReader("abcdefghijklmnop"), minTestReadBufferSize)
	...
	if _, err := buf.Read(p[0:]); string(p[0:]) != "ghijklmnop" || err != nil {
		t.Fatalf("want %q got %q, err=%v", "ghijklmnop", string(p[0:minTestReadBufferSize]), err)
	}
...
func TestLineTooLong(t *testing.T) {
	data := make([]byte, 0)
	for i := 0; i < minTestReadBufferSize*5/2; i++ {
		data = append(data, '0'+byte(i%10))
	}
	buf := bytes.NewReader(data)
	l := NewReaderSize(buf, minTestReadBufferSize)
	line, isPrefix, err := l.ReadLine()
	if !isPrefix || !bytes.Equal(line, data[:minTestReadBufferSize]) || err != nil {
		t.Errorf("bad result for first line: got %q want %q %v", line, data[:minTestReadBufferSize], err)
	}
	data = data[len(line):]
	line, isPrefix, err = l.ReadLine()
	if !isPrefix || !bytes.Equal(line, data[:minTestReadBufferSize]) || err != nil {
		t.Errorf("bad result for second line: got %q want %q %v", line, data[:minTestReadBufferSize], err)
	}
	data = data[len(line):]
	line, isPrefix, err = l.ReadLine()
	if isPrefix || !bytes.Equal(line, data[:minTestReadBufferSize/2]) || err != nil {
		t.Errorf("bad result for third line: got %q want %q %v", line, data[:minTestReadBufferSize/2], err)
	}
	line, isPrefix, err = l.ReadLine()
	if isPrefix || err == nil {
...
func TestReadEmptyBuffer(t *testing.T) {
	l := NewReaderSize(new(bytes.Buffer), minTestReadBufferSize)
	line, isPrefix, err := l.ReadLine()
	if err != io.EOF {
...
func TestLinesAfterRead(t *testing.T) {
	l := NewReaderSize(bytes.NewReader([]byte("foo")), minTestReadBufferSize)
...
func testReadLineNewlines(t *testing.T, input string, expect []readLineResult) {
	b := NewReaderSize(strings.NewReader(input), minTestReadBufferSize)
...
func TestReaderSize(t *testing.T) {
	if got, want := NewReader(nil).Size(), defaultTestBufSize; got != want {
...
func TestWriterSize(t *testing.T) {
	if got, want := NewWriter(nil).Size(), defaultTestBufSize; got != want {
